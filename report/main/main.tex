...
На рис. \ref{pic:CUI_Surrender} изображена ситуация, в которой чёрный игрок сдался. Выводятся сообщения о том, что чёрный игрок сдался, а белый игрок победил, и игра закончена. Ниже следует предложение начать новую игру или выйти в меню.
...
Разработка велась в цикле непрерывной интеграции (continuous integration - ci). Кроме того, она сопровождалась ручным и автоматическим(модульным и функциональным) тестированием, статическим и динамическим анализом кода, просмотрами кода (code review) и демонстрациями.
...
Непрерывная интеграция автоматизировалась с помощью платформы Jenkins.
...
\addcontentsline{toc}{subsubsection}{Компиляция и сборка}

Программа компилировалась 64 битным компилятором \textbf{GCC}.\\
...
\subsubsection*{Тесты}
\addcontentsline{toc}{subsubsection}{Тесты}
...
На графике видно, что 3 теста не завершились успешно. Это обусловлено тем, что еще не реализована функциональность, которая ими тестируется.

\subsubsection*{Статический анализ кода}
\addcontentsline{toc}{subsubsection}{Статический анализ кода}
...
На графике видно, что cppchek предупреждает об стилевых деффектах кода. Автор не согласен с ними. Еще cppcheck предупреждает о том, что не инициализируются переменные, когда они инициализируются при помощи списков инициализации и фигурных скобок. Такие предупреждения автор также игнорирует.

\subsubsection*{Динамический анализ кода}
\addcontentsline{toc}{subsubsection}{Динамический анализ кода}
...
\addcontentsline{toc}{subsubsection}{Подсчёт числа строк}
...
Итак, несмотря на то, что автор не успел реализовать всё, что было задумано, им был получен большой опыт. Задачи обозначены, приоритеты расставлены. Планируется реализовать проект в полной мере.
